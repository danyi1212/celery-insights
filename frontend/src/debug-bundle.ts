...
export async function importSurrealNative(config: Config, db: Surreal, sourcePath: string): Promise<void> {
    await resolveQueryResult(db.query("DELETE FROM task; DELETE FROM event; DELETE FROM worker; DELETE FROM ingestion_lock;"))
...
function reviveDateFields(table: "task" | "event" | "worker", record: Record<string, unknown>): Record<string, unknown> {
...
export async function importSurrealData(db: Surreal, data: SurrealExport): Promise<void> {
    ...
    for (const [table, records, key] of [
        ...
        chunks.forEach((chunk, index) => {
            ...
            queryLines.push(`FOR $record IN $${paramName} { UPSERT type::record('${table}', $record.id) CONTENT $record.data; }`)
        })
    }
...
export async function parseDebugSnapshot(bundlePath: string): Promise<ParsedDebugSnapshot> {
    ...
    return {
        ...
        sourceLogs: {
            ...
            python: entries.has("source/logs/python.log")
                ? await readTextFromZip(bundlePath, "source/logs/python.log")
                : "",
...
export async function createDebugBundleArchive(options: {
    ...
    try {
        ...
        if (options.replaySnapshot) {
            ...
            await writeJson(path.join(stagingDir, "source/config/effective-config.json"), redactConfig(options.config, options.includeSecrets))
