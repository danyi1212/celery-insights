...
export interface UseLiveQueryOptions<T> {
    ...
    liveTable: string
    /** Optional WHERE clause for filtering live notifications */
    liveCondition?: string
    /** Query parameter bindings for both initial query and live condition */
...
export function useLiveQuery<T extends { id: unknown }>(options: UseLiveQueryOptions<T>): UseLiveQueryResult<T> {
    const { initialQuery, liveTable, bindings, orderBy, limit, enabled = true } = options
    ...
    // Use refs for orderBy/limit to avoid recreating callbacks when these change.
        ...
        (message: LiveMessage) => {
            ...
            if (action === "CREATE") {
                ...
                setData((prev) => {
                    ...
                    if (idx === -1) {
                        // Record not in current list — might match after update, add it
                        const next = [...prev, record]
                        return applyOrderAndLimit(next)
                    }
                    const next = [...prev]
    ...
    const startLiveSubscription = useCallback(async () => {
        ...
        try {
            const subscription = await db.live<T>(liveTable)
