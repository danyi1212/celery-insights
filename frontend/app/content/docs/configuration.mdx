...
Absolute path to a mounted `debug bundle v2` zip file. When this is set, Celery Insights starts in an offline, read-only replay mode that restores the bundled `task`, `event`, and `worker` export into the embedded SurrealDB instance before serving traffic.

Replay mode forces ingestion off, skips leader election, ignores `SURREALDB_EXTERNAL_URL`, and keeps all mutating settings actions disabled in the UI.

`SURREALDB_STORAGE` still applies here. Leave it at the default for disposable in-memory replay, or set a local embedded storage path if you want the restored snapshot to persist across restarts.

Go to [/documentation/debug-bundles](/documentation/debug-bundles) for the full support and replay workflow.
