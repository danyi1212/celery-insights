...
describe("LeaderElection", () => {
  ...
  let onBecomeLeaderCalls = 0
  let onLoseLeadershipCalls = 0

  beforeEach(() => {
    vi.useFakeTimers()
    mockDb = createMockDb()
    onBecomeLeaderCalls = 0
    onLoseLeadershipCalls = 0
  })
  ...
  function createElection(configOverrides: Partial<Config> = {}): LeaderElection {
    return new LeaderElection({
      ...
      onBecomeLeader: () => {
        onBecomeLeaderCalls += 1
      },
      onLoseLeadership: () => {
        onLoseLeadershipCalls += 1
      },
    })
  }
  ...
  describe("ingestion disabled", () => {
    it("returns read-only status without querying SurrealDB", async () => {
      ...
      expect(onBecomeLeaderCalls).toBe(0)
      await election.stop()
    })
  })
  ...
  describe("leader election disabled", () => {
    it("assumes leader role immediately", async () => {
      ...
      expect(onBecomeLeaderCalls).toBe(1)
      expect(mockDb.query).not.toHaveBeenCalled()
      await election.stop()
    })
  })
