        ...
        {isDemo ? (
          <div className="rounded-2xl border bg-background/60 p-5">
            ...
            <p className="mt-3 max-w-3xl text-sm text-muted-foreground">
              This browser is using sample data, so Settings does not query a real Celery Insights instance, SurrealDB
              deployment, or ingestion runtime from this section.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <SummaryTile
                ...
                badge={data ? <Badge variant="outline">{formatDurability(durability)}</Badge> : undefined}
              />
              <SummaryTile
                title="Stored data"
              ...
              <SummaryTile
                ...
                hint={data ? `Up for ${formatSecondsDurationLong(data.uptime)}` : "Checking process runtime."}
              />
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              />
              <DetailGridItem label="Engine" value={data ? formatStorageEngine(data.surrealdb.storage) : "—"} />
            </div>
            ...
            <div className="rounded-2xl border bg-background/40">
              ...
              {showAdvanced && (
                <div className="grid gap-4 border-t px-4 py-4 md:grid-cols-2 xl:grid-cols-3">
                  <DetailGridItem label="SurrealDB endpoint" value={data?.surrealdb.endpoint ?? "—"} />
                  ...
                  <DetailGridItem label="Dropped events" value={data?.ingestion.dropped_events ?? "—"} />
                  <DetailGridItem label="Flushes" value={data?.ingestion.flushes_total ?? "—"} />
                  <DetailGridItem label="Events ingested" value={data?.ingestion.events_ingested_total ?? "—"} />
                  <DetailGridItem
