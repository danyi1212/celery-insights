...
const mdxComponents = {
  ...
  th: ({ className, ...props }: React.ComponentProps<"th">) => (
    <th className={cn("px-3 py-3 text-sm font-semibold text-foreground first:pl-0 last:pr-0", className)} {...props} />
  ),
  ...
  return (
    <div className="space-y-2 border-t border-border/70 pt-4">
      ...
      >
        ...
        {copyState === "copied" ? "Copied Markdown" : copyState === "error" ? "Copy Failed" : "Copy as Markdown"}
      </Button>
      {source?.path ? (
        <a
...
export default function DocsPage({
  ...
  const tocHeadings = useMemo(() => headings.filter((heading) => heading.level <= tocMaxDepth), [headings, tocMaxDepth])
