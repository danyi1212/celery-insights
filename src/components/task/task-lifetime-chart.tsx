...
const TaskLifetimeChart: React.FC<TaskLifetimeChartProps> = ({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, idx: number) => {
      if (e.key === "ArrowRight" && idx < phases.length - 1) {
        e.preventDefault()
        const next = containerRef.current?.querySelector(`[data-phase-index="${idx + 1}"]`) as HTMLElement | null
        next?.focus()
      } else if (e.key === "ArrowLeft" && idx > 0) {
        e.preventDefault()
        const prev = containerRef.current?.querySelector(`[data-phase-index="${idx - 1}"]`) as HTMLElement | null
        prev?.focus()
      }
    },
    [phases.length],
  ...
  return (
    <div className={cn("flex flex-col w-full gap-3", className)}>
      ...
      >
        {/* The stacked bar */}
        <div className="flex h-10 w-full overflow-hidden rounded-md" role="group" aria-label="Task lifecycle phases">
          {phases.map((phase, idx) => {
            const widthPct =
              totalDurationMs > 0
                ? Math.max((minPhaseWidth / (containerWidth || 400)) * 100, (phase.durationMs / totalDurationMs) * 100)
                : 100 / phases.length
            const isHovered = hoveredPhase === idx
            const showLabel = showDurationLabels && containerWidth > 0 && (widthPct / 100) * containerWidth > 48
