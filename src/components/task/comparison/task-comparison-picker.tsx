...
const TaskComparisonPicker: React.FC<TaskComparisonPickerProps> = ({
  ...
  const filteredCandidates = useMemo(() => {
    ...
    return candidates.filter(
      (t) =>
        t.id.toLowerCase().includes(q) || (t.type || "").toLowerCase().includes(q) || t.state.toLowerCase().includes(q),
    )
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-lg">{activeSlot ? `Select ${activeSlot} task` : "Select a task"}</CardTitle>
