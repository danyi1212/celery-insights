...
async function insertTaskEvent(
  ...
  if (eventType === "task-sent" || eventType === "task-received") {
    ...
    setClauses.push(`kwargs = IF last_updated IS NONE OR <datetime>$ts >= last_updated THEN $kwargs ELSE kwargs END`)
  ...
  if (extra?.result !== undefined) {
    setClauses.push(`result = IF last_updated IS NONE OR <datetime>$ts >= last_updated THEN $result ELSE result END`)
    params.result = String(extra.result)
  }
  if (extra?.runtime !== undefined) {
    setClauses.push(`runtime = IF last_updated IS NONE OR <datetime>$ts >= last_updated THEN $runtime ELSE runtime END`)
  ...
  if (task.parentId && (eventType === "task-sent" || eventType === "task-received")) {
    await db.query(`UPDATE type::record('task', $parentId) SET children = array::union(children ?? [], [$childId])`, {
